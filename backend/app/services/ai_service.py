...
"""
            ...
            lines = ["## Headline", f"Sector performance summary for {target} across Sales Pipeline and Operational Execution.\n"]
            ...
            wo_only = metrics.get("wo_only_customers", [])
            wo_only_cnt = metrics.get("wo_only_customers_count", len(wo_only))
            total_matched = metrics.get("total_matched_customers", 0)

            lines = ["## Headline", f"Cross-board matching identified {wo_only_cnt} active execution clients with active Work Orders but zero active sales pipeline.\n"]
            lines.append("## Key Metrics")
            lines.append(f"- Active Work Order Clients with No Active Pipeline: {wo_only_cnt}")
            lines.append(f"- Total Matched Accounts Across Boards: {total_matched}")

            lines.append("\n## Priorities")
            if wo_only:
                for idx, c in enumerate(wo_only[:5], 1):
                    val = c.get("order_value_formatted", "₹0")
                    lines.append(f"{idx}. {c.get('customer', 'Account')} — Order Value: {val} (Active execution with no open deal)")
            else:
                lines.append("- All active work order clients have active open deals in pipeline.")

            lines.append("\n## Risks & Data Caveats")
            lines.append("- Expansion risk: Executing projects for accounts without active renewal or upsell deals risks revenue drop-off upon project completion.")
            lines.append("- Account management gap: BD team is not actively positioning follow-on contracts for existing operational clients.")

            lines.append("\n## Recommended Actions")
            lines.append("1. Up-sell campaign — Launch target account outreach to active execution clients.")
            lines.append("2. Renewal planning — Require BD team to log renewal pipeline 60 days before work order completion.")

            lines.append("\n## Bottom Line")
            lines.append("Active work order accounts represent high-intent renewal opportunities with zero customer acquisition cost.")
            return "\n".join(lines)

        elif intent == "opportunity_analysis":
            opps = metrics.get("top_opportunities", [])
            lines = ["## Headline", f"Top {len(opps)} weighted opportunities ranked deterministically by expected value.\n"]
            lines.append("## Key Metrics")
            for o in opps:
                lines.append(f"- Rank {o['rank']}: {o['deal_name']} ({o['customer']}) — Deal Value: {o['deal_value_formatted']} | Probability: {o['probability_pct']}% | Weighted: {o['weighted_formatted']}")
            lines.append("\n## Priorities")
            lines.append("1. Secure commercial sign-off on top ranked weighted opportunities.")
            lines.append("2. Ensure executive touchpoints on deals over ₹1 Cr value.")
            return "\n".join(lines)

        elif intent == "work_order_analysis":
            tot_val = metrics.get("total_order_value_formatted", "₹0")
            tot_cnt = metrics.get("total_work_orders", 0)
            active_cnt = metrics.get("active_work_orders", 0)

            return f"""## Headline
Skylark actively executes {active_cnt} active work orders totaling {tot_val} in total order value (Total logged work orders: {tot_cnt}).

## Key Metrics
- Active Order Value: {tot_val}
- Active Work Orders: {active_cnt}
- Completed Work Orders: {metrics.get('completed_work_orders', 0)}

## What the Data Shows
- Operational execution is actively progressing across primary service sectors.

## Recommended Actions
1. Milestone tracking — Ensure execution milestones are updated weekly to prevent billing delays.
2. Resource planning — Align field operations team allocation with priority high-value work orders.
"""
            ...
            lines.append("## Key Metrics")
            lines.append(f"- High Deal Value Threshold: >= {h_thresh}")
            lines.append(f"- Low Closure Probability Threshold: <= {l_prob}%")
...
"""
            ...
            return f"""## Headline
Consolidated Skylark BI analysis for query: "{question}".

## Key Metrics
- Open Pipeline Value: {open_pipe}
- Active Open Deals: {open_cnt}

## What the Data Shows
- Active operations and deal funnels demonstrate steady commercial engagement across key sectors.

## Recommended Actions
1. Accelerate high-value deal conversion and monitor milestone completion.

## Bottom Line
Execution focus on key opportunities will drive target revenue achievement.{dq_section}
"""
